...
class FrameWork(AbstractBaseClass):
    ...
    def __init__(self,
        ...
        if feature_acquisition is None:
            feature_acquisition = NoActiveFeatureAcquisition(target_col_name=target_col_name, 
                budget_manager=NoBudgetManager(budget_threshold=1.0), acq_set_size=0)
        ...
        if feature_acquisition is not None:
            ...
            feature_set_str = feature_acquisition.acq_set_size
            budget_man_str = feature_acquisition.budget_manager.get_name()
            budget_str = feature_acquisition.get_budget_string()

        if not os.path.isdir(results_path):            
            self.dir_result = self.create_dir(self.dir, "results")
        else:
            self.dir_result = self.create_dir(results_path, "results")
        self.dir_result = self.create_dirs(self.dir_result, pre_folder_name)
        self.dir_result = self.create_dir(self.dir_result, feature_acq_str)
        self.dir_result = self.create_dir(self.dir_result, str(feature_set_str))
        ...
        self.dir_result = self.create_dirs(self.dir_result, post_folder_name)
    ...
    def create_dirs(path, dirs):
        allpath = FrameWork.splitall(dirs)
        for dir in allpath:
            path = FrameWork.create_dir(path, dir)
        return path
